...
v_add_optimized_adjacent(double* x, double* y, double* z) 
{
    #pragma omp parallel
    {
        ...
        int thrd_num = omp_get_num_threads();

        for (int i = id; i < ARRAY_SIZE; i = i + thrd_num )
            z[i] = x[i] + y[i];
    }
}
...
v_add_optimized_chunks(double* x, double* y, double* z) 
{
    #pragma omp parallel
    {
        ...
        int chunk_size = ARRAY_SIZE / thrd_num;
        int tailcase = ARRAY_SIZE - (chunk_size * thrd_num);

        int chunk_start = chunk_size * id;
        int chuck_end = chunk_size * (id + 1);

        if (ARRAY_SIZE - chuck_end == tailcase)
            chuck_end += tailcase;

        for(int i = chunk_start; i < chuck_end; i++)
            z[i] = x[i] + y[i];
    }
}
// END PART 1 EX 2
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
    ...
    #pragma omp parallel
    {
        double thread_lvl_sum = 0;
        #pragma omp for
        for (int i = 0; i < arr_size; i++)
            thread_lvl_sum += x[i] * y[i];

        #pragma omp critical
        global_sum += thread_lvl_sum;
    }
