...
main:
    ...
    jal ra, map # void map(struct node *head<a0>, int (*f)(int)<a1>)
...
map:
    ...
    addi sp, sp, -20 # Make space to store a0(head pointer), a1(function pointer), ra(return address
    sw a0, 0(sp)	# Store a0
    sw a1, 4(sp)	# Store a1
    sw s0, 8(sp)	# Store s0 address of node
    sw s1, 12(sp)	# Store s1 address of argument function
    ...
    jalr s1 # Jump to function pointer function

    # Store the returned value back into the node
    # Where can you assume the returned value is?
    sw a0, 0(s0) 	# node->value = a0 (returned value)
    ...
    add a1, s1, x0 	# a1 = s1 (address of function)
