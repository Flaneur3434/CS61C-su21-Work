...
Matrix61c_add(Matrix61c* self, PyObject* args) 
{
    ...
    {
        PyErr_SetString(PyExc_TypeError, "Arguments must be of type numc.Matrix");
        return NULL;
    }

    matrix *new_mat;
    int alloc_error = allocate_matrix(&new_mat, self->mat->rows, self->mat->cols);
    ...
    int add_error = add_matrix(new_mat, self->mat, ((Matrix61c *)args)->mat);
    ...
    Matrix61c *ret_obj = (Matrix61c *) Matrix61c_new(&Matrix61cType, NULL, NULL);
    ret_obj->mat = new_mat;
    ret_obj->shape = PyTuple_Pack(2, PyLong_FromLong(new_mat->rows), PyLong_FromLong(new_mat->cols));
...
Matrix61c_sub(Matrix61c* self, PyObject* args) 
{
    ...
    int sub_error = sub_matrix(new_mat, self->mat, ((Matrix61c *)args)->mat);
...
Matrix61c_multiply(Matrix61c* self, PyObject *args) 
{
    ...
    matrix *new_mat;
    int alloc_error = allocate_matrix(&new_mat, self->mat->rows, ((Matrix61c *)args)->mat->cols);
    ...
    int mul_error = mul_matrix(new_mat, self->mat, ((Matrix61c *)args)->mat);
 ...
 */
static PyObject *
Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) 
{
    if(!PyObject_TypeCheck(self, &Matrix61cType))
    {
        PyErr_SetString(PyExc_TypeError, "Arguments must be of type numc.Matrix");
    ...
    if (!PyLong_Check(pow))
    {
        PyErr_SetString(PyExc_TypeError, "Power must be a integer");
        return NULL;
    }
    ...
    int pow_error = pow_matrix(new_mat, self->mat, (int) PyLong_AsLong(pow));
    ...
    {
        PyErr_SetString(PyExc_ValueError, "Invalid exponent");
        return NULL;
    }
...
{
    PyObject *row = NULL; PyObject *col = NULL; PyObject *val = NULL;
    
    int unpack_err = PyArg_UnpackTuple(args, "set_value", 3, 3, &row, &col, &val);

    if (unpack_err == 0)
    {
        PyErr_SetString(PyExc_TypeError, "Number of argunments parsed from args is not 3");
        Py_RETURN_NONE;
    }

    if (!PyLong_Check(row) || !PyLong_Check(col))
    {
        PyErr_SetString(PyExc_TypeError, "Row or Column value is not an integers");
        Py_RETURN_NONE;
    }

    if (!PyFloat_Check(val) && !PyLong_Check(val))
    {
        PyErr_SetString(PyExc_TypeError, "Val is not an int or a float");
        Py_RETURN_NONE;
    }
    
    if (self->mat->rows < (int) PyLong_AsLong(row) || self->mat->cols < (int) PyLong_AsLong(col))
    {
        PyErr_SetString(PyExc_IndexError, "Row or column passed in are out of range");
        Py_RETURN_NONE;
    }

    set(self->mat, (int) PyLong_AsLong(row), (int) PyLong_AsLong(col), PyFloat_AsDouble(val));
...
{
    PyObject *row = NULL; PyObject *col = NULL;
    int unpack_err = PyArg_UnpackTuple(args, "get_value", 2, 2, &row, &col);

    if (unpack_err == 0)
    {
        PyErr_SetString(PyExc_TypeError, "Number of argunments parsed from args is not 2");
        return NULL;
    }

    if (!PyLong_Check(row) || !PyLong_Check(col))
    {
        PyErr_SetString(PyExc_TypeError, "Row and Colum values are not integers");
        return NULL;
    ...
    if (self->mat->rows < (int) PyLong_AsLong(row) || self->mat->cols < (int) PyLong_AsLong(col))
    {
        PyErr_SetString(PyExc_IndexError, "Row and column passed in are out of range");
        return NULL;
    }

    double get_value = get(self->mat, (int) PyLong_AsLong(row), (int) PyLong_AsLong(col));

    return PyFloat_FromDouble(get_value);
}
...
static PyMethodDef
Matrix61c_methods[] = {
    {"get", (PyCFunction) &Matrix61c_get_value, METH_VARARGS, "gets value from a matrix"},
    {"set", (PyCFunction) &Matrix61c_set_value, METH_VARARGS, "sets value in a matrix"},
    {NULL, NULL, 0, NULL}
};
